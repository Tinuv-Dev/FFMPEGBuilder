...
extension Library {
    ...
    var libSourceDirectory: URL {
        URL(string: FFMPEGBuilder.buildDirectory+"/\(self.rawValue)"+"-"+"source"+"-"+"\(self.libVersion)")!
    }

    func thin(platform: PlatformType, arch: ArchType) -> URL {
        URL(string: FFMPEGBuilder.buildDirectory+"/\(self.rawValue)-build"+"/\(platform.rawValue)"+"/thin"+"/\(arch.rawValue)")!
    }

    func scratch(platform: PlatformType, arch: ArchType) -> URL {
        URL(string: FFMPEGBuilder.buildDirectory+"/\(self.rawValue)-build"+"/\(platform.rawValue)"+"/scratch"+"/\(arch.rawValue)")!
    }

    func framework(platform: PlatformType, framework: String) -> URL {
        URL(string: FFMPEGBuilder.buildDirectory+"/\(self.rawValue)-frameworks"+"/\(platform.rawValue)"+"/\(framework).framework")!
    }

    func xcFramework(framework: String) -> URL {
        URL(string: FFMPEGBuilder.distDirectory+"/\(framework).xcframework")!
    }
